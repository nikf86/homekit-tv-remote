...
2. Expose the integration's `media_player` to HomeKit Bridge **in accessory mode** — this is what enables the iOS remote widget:
...
2. Press **1e. Test Command** to verify it works
3. Press **1f. Save Input**
...
| **1d. HAP Identifier** | The number your TV reports for this input — read it from **1g. Current Identifier** while the input is active |
| **1e. Test Command** | Fires the command immediately without saving — works for all input types |
| **1f. Save Input** | Saves the input |
| **1g. Delete Last Input** | Removes the last saved input |
| **1g. Current Identifier** | Live read-out of the TV's current input and its HAP number |
...
The Apple TV integration requires `select_source` instead of `play_media`. Turn on **1. Apple TV App** before saving an Apple TV app input. Optionally turn on **1. Apple TV Input** to also switch the TV's HDMI port automatically before launching the app (requires HAP Identifier to be set).
...
**HDMI / CEC input**
Switch to the input on your TV, read its number from **1g. Current Identifier**, then fill in:
`1b. Command` = `input_9` · `1d. Input Type` = `hap`

**Third-party remote (e.g. Bravia)**
`1b. Command` = `Hdmi2` · `1d. Input Type` = `remote.bravia_kd_55xg9505` · `1d. HAP Identifier` = `3`

**App via media player (e.g. Bravia Netflix)**
`1c. App Name` = `Netflix` · `1d. Input Type` = `media_player.bravia_kd_55xg9505`

**Apple TV app**
`1. Apple TV App` = On · `1c. App Name` = `Netflix` · `1d. Input Type` = `media_player.ng_apple_tv` · `1d. HAP Identifier` = `9`
