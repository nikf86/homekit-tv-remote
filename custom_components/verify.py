...
import shim  # noqa: F401  — backport newer HA names onto the installed version
...
from custom_components.homekit_tv_remote.remote import TVRemote, _discover, HAP_SUCCESS
...
# ─── 10. Regressions fixed in 2.4.0 ───────────────────────────────────────────

async def section_10() -> None:
    print("\n10. 2.4.0 regression fixes")
    hass = FakeHass()
    entry = make_entry()
    remote = TVRemote(hass, entry, FakeConn(), found, "media_player.hk_tv")
    remote.async_write_ha_state = lambda: None
    ...
    # Ordering: rapid presses must arrive in the order they were made.
    writes.clear()
    for key in (4, 5, 6, 7, 8):
        await remote.async_send_command([str(key)])
    await remote._queue.join()
    check("rapid presses keep their order", [w[2] for w in writes], [4, 5, 6, 7, 8])

    # A failing write must surface, not be recorded as success.
    class FailingConn:
        async def put_characteristics(self, payload):
            raise RuntimeError("communication failure")

    bad = TVRemote(hass, entry, FailingConn(), found, "media_player.hk_tv")
    bad.async_write_ha_state = lambda: None
    bad._last_error = HAP_SUCCESS
    await bad.async_send_command(["8"])
    await bad._queue.join()
    check("failed write is not recorded as success", bad._last_error != HAP_SUCCESS, True)
    check("failed write is classified", bad._last_error, -70402)

    # hold_secs is accepted and ignored, and no longer blocks the fast path.
    writes.clear()
    await remote.async_send_command(["8"], hold_secs=3)
    await remote._queue.join()
    check("hold_secs no longer delays or holds", writes, [(1, rk_iid, 8)])

    # Unreachable is not off.
    media = HomeKitTVMediaPlayer(hass, entry, "media_player.hk_tv")
    media.async_write_ha_state = lambda: None
    media._apply_hk_state(SimpleNamespace(state="on", attributes={"source": "HDMI 1"}))
    check("TV reports on", media.state, MediaPlayerState.ON)
    media._apply_hk_state(SimpleNamespace(state="unavailable", attributes={}))
    check("blip does not turn the TV off", media.state, MediaPlayerState.ON)
    check("blip does not clear the input", media._tv_source, "HDMI 1")
    media._apply_hk_state(SimpleNamespace(state="off", attributes={"source": "HDMI 1"}))
    check("a real off is still off", media.state, MediaPlayerState.OFF)

    # Same for the remote entity.
    remote._attr_is_on = True
    remote._current_source = "HDMI 1"
    remote._apply_hk_state(SimpleNamespace(state="unavailable", attributes={}))
    check("remote holds power through a blip", remote.is_on, True)
    check("remote holds input through a blip", remote._current_source, "HDMI 1")



asyncio.run(section_10())

print()
if FAILURES:
    print(f"{len(FAILURES)} FAILURE(S): " + '; '.join(FAILURES))
    sys.exit(1)
print('ALL CHECKS PASSED (including 2.4.0 regressions)')
